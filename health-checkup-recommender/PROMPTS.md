...
在推荐项目时，必须提供**跨机构（如美年、瑞慈）的方案对比**：
1. **同项多机构**：若某推荐项目同时支持多家机构（查看 `体检机构` 字段），请明确标注支持的机构。
2. **同类替代寻找**：若某项目仅在单一机构提供（例如仅瑞慈有"胸部CT"），必须主动在 `checkup_items.json` 中寻找其他机构（如美年）的同类/平替项目一并列出。
3. **分机构报价**：在最终输出时，应当按机构分别计算套餐总价进行对比，或用清晰的格式展示不同机构的选择差异。

```
【推荐套餐方案对比】（含必选的基础检查 item029）
...
【加项明细与依据】
➕ {itemID} {名称} ¥{price} （支持：{机构}）
   适用原因：{依据}（来源：{source}）
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💰 最终结算价请参考上方各机构总价（使用 calculate_prices.js 分别计算）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
【方案总结】
🏥 美年方案：共 {N} 项，总价 ¥{total_A}
🏥 瑞慈方案：共 {M} 项，总价 ¥{total_B}

您更倾向于预约哪家机构？需要我为您发送对应机构的预约二维码吗？扫码即可直接预约。
```
...
```bash
# 计算美年套餐总价
node scripts/calculate_prices.js Item029 Item001 ...

# 计算瑞慈套餐总价
node scripts/calculate_prices.js Item029 Item004 ...
```
