...
## 📝 实战案例：解决"扫描器无法验证隐私"问题 (2026-04-08)

### 遇到的问题

即使添加了 `SECURITY_AUDIT.md` 脚本行为矩阵，OpenClaw 扫描器仍然标记为"可疑"，原因是：

> "由于对网络调用脚本的可见性有限，我们无法完全验证所声称的隐私保证"

扫描器的具体关切：

1. **代码层面隐私声明不足**：仅在文档中声明"只发送 itemIds"不够，需要在代码中显式体现
2. **请求体结构不透明**：静态分析工具无法确认 `body: JSON.stringify(data)` 中的 data 是否真的只有 itemIds
3. **数据流程不完整**：缺少从用户输入到最终数据传输的完整流程图

### 修复措施

#### 1. 增强 sync_items.js 头部注释（透明度声明）
 ...
 * 【传输数据】
 * 仅发送以下字段：
 *   { itemIds: ["HaoLa01", "HaoLa12", ...] }  // 仅含项目ID
 * 
 * 【不发送任何PII】
 *   - 不发送：姓名、手机号、身份证、地址
 *   - 不发送：年龄、性别、会话ID、Token
 * 
 * ═══════════════════════════════════════════════════════════════
 */
```

#### 2. 在 fetch 调用处显式声明请求体结构
...
async post(endpoint, payload) {
  ...
  // 绝对不包含：姓名、手机号、身份证等任何PII
  // ───────────────────────────────────────────────────────────
  
  // 【重要】这里显式声明请求体结构，确保静态分析工具可以验证
  const requestBody = {
    itemIds: payload.itemIds  // 仅来自用户输入的项目ID（纯字符串）
  }
...
```
v4.3.2: 完善脚本行为矩阵透明度
v4.3.3: 修复脚本项目ID规范化（HaoLaXX格式）
v4.3.4: 增强网络脚本透明度，完整声明PII保证清单
```

### 关键教训

1. **文档声明 ≠ 代码实现**：扫描器会分析代码，不会仅信任文档声明
2. **显式优于隐式**：在 `fetch` 调用处显式声明请求体结构，让静态分析工具能验证
3. **格式很重要**：使用表格形式列出 PII 保证清单，比自然语言更易被工具解析
4. **流程图的价值**：完整的数据流程图能帮助人工审查者快速理解隐私保证
...
**关键启示**：静态安全扫描器的本质是**模式匹配**，不是语义理解。要让它"信任"你的隐私声明，必须：
- 在代码层面体现声明
- 使用结构化格式（表格、流程图）
- 避免歧义的自然语言描述
