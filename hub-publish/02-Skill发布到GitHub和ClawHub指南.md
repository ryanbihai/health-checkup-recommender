...
```bash
clawhub publish "完整路径" --version 4.1.0 --changelog "更新说明"
```

**示例**：

```bash
clawhub publish "c:/IT/00 工具和探索/clawhub/health-checkup-recommender" --version 4.1.0 --changelog "安全修复"
```

### 5.3 查看发布结果

```bash
clawhub inspect health-checkup-recommender
```
...
| 发布 Skill | `clawhub publish <path> --version <ver>` |
...
echo "🚀 发布到 ClawHub..."
clawhub publish "${SKILL_PATH}" --version "${VERSION}" --changelog "${CHANGELOG}"
