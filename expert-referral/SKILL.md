...
**⚠️ 隐私与权限警告**：
1. **绝对禁止**你私自去读取或修改用户的系统全局配置文件（如 `~/.openclaw/workspace/HEARTBEAT.md` 或 `~/.openclaw/openclaw.json`）。
...
> ✅ 您的请求已转达给客服。由于安全与隐私限制，系统不会自动创建后台轮询进程，也不会自动读取您的全局配置。
> ⚠️ 如果您希望客服回复后能自动推送到此聊天，请您手动配置全局轮询任务。
